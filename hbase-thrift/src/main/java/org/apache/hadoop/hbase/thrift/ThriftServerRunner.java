...
public class ThriftServerRunner implements Runnable {
  ...
  private void setupHTTPServer() throws IOException {
    ...
    if (MAPR_SASL.equalsIgnoreCase(authMethod)) {
      ...
      ctxHandler.addFilter(authFilter, "/*", EnumSet.of(DispatcherType.REQUEST));
    }
    ...
    if(conf.getBoolean(THRIFT_SSL_ENABLED, false)) {
      ...
      String[] includeCiphers = conf.getStrings(
              THRIFT_SSL_INCLUDE_CIPHER_SUITES, org.apache.commons.lang.ArrayUtils.EMPTY_STRING_ARRAY);
  ...
  TTransportFactory createTransportFactory() throws Exception {
    ...
    RpcAuthMethod authMethod = serviceUGI.getRpcAuthMethodList().get(0);
    ...
    if (SaslRpcServer.AuthMethod.KERBEROS.getMechanismName().equals(mechanism)) {
      ...
      protocol = SecurityUtil.getUserFromPrincipal(thriftKerberosPrincipal);
    }
    ...
    try {
      ...
      if (SaslRpcServer.AuthMethod.KERBEROS.getMechanismName().equals(mechanism)) {
        ...
        return callbackHandlerFactory.getImplementorInstance(
                ...
                new Object[] {serviceUGI.getSubject(), serviceUGI.getUserName()},
  ...
  public static class HBaseHandler implements Hbase.Iface {
    ...
    public List<String> getTableNamesByPath(String regex) throws IOError, TException {
      try {
        HTableDescriptor[] tables = this.getAdmin().listTables(regex);
